...
import org.n52.iceland.ogc.ows.DisableableExtension;
 ...
 */
public class JsonCapabiltiesExtensionService extends AbstractJsonDao implements CapabilitiesExtensionService {
    ...
    protected void checkOfferingName(String offering)
            throws NoSuchOfferingException {
        if (!this.contentCacheController.getCache().hasOffering(offering)) {
    ...
    private static <T extends SwesExtension<?>> Collector<T, ?, Map<String, T>> swesExtensionCollector() {
        return toMap(SwesExtension<?>::getIdentifier, Function.identity());
    }
    ...
    private static abstract class AbstractSwesExtension extends SwesExtensionImpl<String>
